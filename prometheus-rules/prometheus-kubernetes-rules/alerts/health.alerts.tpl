...
- name: kubernetes.alerts
  ...
  - alert: KubernetesPodRestartingTooMuch
    expr: (sum by(pod, namespace, container) (rate(kube_pod_container_status_restarts_total[15m]))) * on (pod) group_left(label_alert_tier, label_alert_service) kube_pod_labels{} > 0
    for: 1h
    labels:
