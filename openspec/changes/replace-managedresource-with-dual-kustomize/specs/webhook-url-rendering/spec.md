...
**Migration**: Upgrading the upstream metal-operator version is performed by editing the `?ref=` value in `system/kustomize/metal-operator-remote/host/base/kustomization.yaml`, `remote/upstream/metal-operator-crds-and-rbac/kustomization.yaml`, and `remote/upstream/metal-operator-webhooks/upstream-no-svc/kustomization.yaml`. The next `kustomize build` (or `kubectl apply -k`) automatically picks up the new upstream content. No regeneration cycle, no commit of pre-rendered output.
