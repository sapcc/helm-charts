...
## Task 15: Close helm-vs-kustomize equivalence gaps (post-PR-review)

**Files:**

- Modify: `system/kustomize/metal-operator-remote/host/base/kustomization.yaml` (add new resource + optionally `commonLabels:`)
- Modify: `system/kustomize/metal-operator-remote/host/base/networkpolicy.yaml` (add `policyTypes`; possibly relabel podSelector)
- Create: `system/kustomize/metal-operator-remote/host/base/networkpolicies.yaml` (the 4 missing NPs ported from helm)
- Update spec deltas: `specs/kustomize-resource-splitting/spec.md` (new ADDED requirement for fleet-uniform NetworkPolicies — already drafted alongside Task 15)
- Update `design.md` "Helm-vs-kustomize equivalence gap analysis" section (already drafted): record the chosen option for items 3+4 cutover decision
- Update `proposal.md` "What changes" section (brief addition)
- Cross-repo: `cc/kube-secrets` companion PR's `verify.md` gains a "Helm-vs-kustomize equivalence verification (PASS WITH WARNINGS)" section (this is the cc/kube-secrets PR's task, but lives in the same workflow)

**Why this section (post-PR-review finding):**

The helm-vs-kustomize diff for `a-qa-de-200`/k3s-admin values surfaced 7 gaps. Per the OQ10 transitivity argument applied during webhook-injector RBAC absorption (Task 14), fleet-uniform values belong in helm-charts. Items 1+2 (NetworkPolicies) are mechanical; items 3+4 (label/Deployment rename) require a cutover-aware decision; item 5 (image SHA) is a per-cluster operational decision (not a code gap); items 6+7 are pivot artifacts already correctly resolved. Full analysis lives in `design.md` "Helm-vs-kustomize equivalence gap analysis" section.
...
- [x] **Step 15.1: Decide cutover strategy for items 3 + 4**
  ...
  - **(b) Accept rename + tested cutover runbook** (cleaner long-term, requires runbook test in non-production)
  - **(c) Bridging `matchExpressions` selector** (most flexible, most temporary cruft)
...
- [x] **Step 15.2: Port the 4 missing NetworkPolicies**
  ...
  2. `metalapi-ingress-from-kube-apiserver-9443`
  3. `kube-apiserver-egress-to-metalapi`
  4. `metalapi-ingress-from-kube-monitoring`

  Substitute the helm template's `{{- include "chart.selectorLabels" . | nindent N }}` with literal labels matching the choice in 15.1:
  - Option (a): `app.kubernetes.io/name: metal-operator-remote, control-plane: controller-manager` (and the `commonLabels:` step in 15.4 is a no-op for these because they're already explicit)
  - Option (b): `app.kubernetes.io/name: metal-operator, control-plane: controller-manager`
  - Option (c): `matchExpressions:` form
...
- [x] **Step 15.3: Add `policyTypes` to the existing NetworkPolicy**
  ...
  - If option (a) was chosen in 15.1, no further change needed (the existing podSelector already uses `app.kubernetes.io/name: metal-operator` which `commonLabels:` will override at build time).
...
- [x] **Step 15.5: Verify build**
  ...
  ```bash
  cd system/kustomize/metal-operator-remote
  kustomize build host/base/ > /tmp/host.yaml
  ```
  Expected:
  - `yq -N 'select(.kind == "NetworkPolicy") | .metadata.name' /tmp/host.yaml | wc -l` → 5
...
- [x] **Step 15.9: Commit and push**
  ...
  git add system/kustomize/metal-operator-remote/host/base/networkpolicies.yaml \
          system/kustomize/metal-operator-remote/host/base/networkpolicy.yaml \
          system/kustomize/metal-operator-remote/host/base/kustomization.yaml \
          openspec/changes/replace-managedresource-with-dual-kustomize/

  git commit -m "feat(metal-operator-remote): close helm-vs-kustomize equivalence gaps

  Add the 4 fleet-uniform NetworkPolicies the kustomize tree was missing
  (metalapi-egress-to-ingress-nginx-controller-tcp-443,
  metalapi-ingress-from-kube-apiserver-9443, kube-apiserver-egress-to-metalapi,
  metalapi-ingress-from-kube-monitoring) and add policyTypes: [Ingress] to the
  existing metalapi-ingress-to-metal-operator-metal-registry-service-tcp-10000
  policy. All 5 NPs now match helm chart's rendered output.

  Cutover decision for items 3+4 (pod selector label + Deployment name): chose
  option <a/b/c> per design.md 'Cutover decision for items 3 + 4'.

  Items 5+6+7 are no-code-change (operational decision / pivot artifacts).

  Tracking: helm-vs-kustomize equivalence verification, post-PR-review
  finding 2026-06-05."
...
- [ ] **Step 15.11: Coordinate with cc/kube-secrets companion PR**

  In the cc/kube-secrets PR's `verify.md`, add a "Helm-vs-kustomize equivalence verification (PASS WITH WARNINGS)" section that:
  - Lists all 7 gaps with their disposition.
  - Cross-links to this repo's `design.md` "Helm-vs-kustomize equivalence gap analysis" section (authoritative gap inventory) and this `tasks.md` Section 17.
  - Flags item 5 (image SHA) explicitly as operator-decision-time (OQ5), not a code gap.
  - Confirms items 1+2+3+4 were addressed in this PR (post-merge cross-link).

  This task lives in cc/kube-secrets; helm-charts only owns the cross-link target.

---

## Task 16: Rebind upstream metal-operator RBAC subjects to Gardener-prefixed ServiceAccount (post-deploy fix)

**Files:**

- Modify: `system/kustomize/metal-operator-remote/remote/upstream/metal-operator-crds-and-rbac/kustomization.yaml` (add `patches:` block with 3 subject-replace patches)
- Update spec delta: `specs/kustomize-resource-splitting/spec.md` (already drafted alongside this task — new ADDED requirement "Upstream metal-operator RBAC bindings rebound to Gardener-prefixed ServiceAccount")
- Update `design.md` with a new section "Three-namespaces-of-controller-manager problem"
- Update `proposal.md` with a brief What-changes bullet
- Cross-repo: a TEST-PHASE block in `cc/kube-secrets//values/kustomize/admin-k3s/qa-de-1/a-qa-de-200/metal-operator-remote/remote/kustomization.yaml` becomes redundant once this lands and must be removed in a follow-up cc/kube-secrets commit

**Why this section (post-deploy fix from `a-qa-de-200`):**

After `kubectl apply -k <overlay>/remote/` lands the upstream RBAC verbatim, the metal-operator-remote controller-manager Pod (running on the seed in the shoot CP namespace, talking to the workerless cluster via remote-kubeconfig) emits a forever-loop of:
...
Pod stays `2/2 Running` but reconciles nothing. Three SA identities collide:

| Where | SA name | Created by |
|---|---|---|
| Workerless, `kube-system` | `controller-manager` (unprefixed) | Upstream `ironcore-dev/metal-operator//config/rbac/service_account.yaml` (consumed verbatim) |
| Workerless, `kube-system` | `metal-operator-controller-manager` (Gardener-prefixed) | Gardener token-injector — created in response to the `serviceaccount.resources.gardener.cloud/name` annotation on the host-side `metal-operator-remote-kubeconfig` Secret |
| Seed, shoot CP namespace | `metal-operator-webhook-injector` | webhook-injector Component (irrelevant here) |

The remote-kubeconfig token authenticates as the Gardener-prefixed SA, but upstream's three bindings (`manager-rolebinding`, `metrics-auth-rolebinding`, `leader-election-rolebinding`) point at the unprefixed `controller-manager`. The helm chart avoids this via fullname-templated SA + binding names that render consistent; the kustomize port loses this consistency without explicit subject patches.

Fix idiom mirrors the sibling `remote/upstream/webhook-injector-rbac/` subtree (which already overrides the `webhook-injector-target` ClusterRoleBinding subjects to the same Gardener-prefixed SA).
...
- [x] **Step 16.1: Edit the kustomization to add subject-replace patches**
  ...
  patches:
    ...
    # rather than the unprefixed upstream default. The Gardener token-injector
    # creates the prefixed SA in response to the annotation on the host-side
    # remote-kubeconfig Secret; the remote-kubeconfig token authenticates as
    # that prefixed SA. Without this rebinding the manager Pod gets forbidden
    # errors on every list/watch — the upstream bindings reference a different
    # identity (the unprefixed `controller-manager`).
    #
    # Mirrors the idiom in remote/upstream/webhook-injector-rbac/kustomization.yaml
    # which rebinds webhook-injector-target ClusterRoleBinding the same way.
    - target:
        ...
        - op: replace
          ...
          value:
            - kind: ServiceAccount
              ...
              namespace: kube-system
    - target:
        # Note: RoleBinding (not ClusterRoleBinding) — leader-election is
        # namespaced to kube-system in upstream config/rbac/.
        kind: RoleBinding
...
- [x] **Step 16.2: Verify the build renders the three bindings with the expected subject**
  ...
  Expected: each binding shows exactly:
  ```yaml
  - kind: ServiceAccount
    name: metal-operator-controller-manager
    namespace: kube-system
...
- [x] **Step 16.6: Commit and push**
  ...
  git commit -m "fix(metal-operator-remote): rebind upstream RBAC subjects to Gardener-prefixed SA

  After kubectl apply -k <overlay>/remote/ lands upstream RBAC verbatim,
  the controller-manager Pod gets forbidden on every list/watch:
  upstream's three bindings (manager-rolebinding, metrics-auth-rolebinding,
  leader-election-rolebinding) point at the unprefixed 'controller-manager'
  SA, but the remote-kubeconfig token authenticates as the Gardener-prefixed
  SA 'metal-operator-controller-manager' (created by the token-injector in
  response to the host-side Secret annotation). Add three subject-replace
  patches to override the bindings to point at the prefixed SA. Mirrors
  the idiom already used in remote/upstream/webhook-injector-rbac/.

  Tracking: post-deploy finding on a-qa-de-200 (2026-06-10).
  Removes the need for a parallel TEST-PHASE block in cc/kube-secrets
  (separate follow-up commit there)."
...
- [ ] **Step 16.8: Coordinate with cc/kube-secrets companion PR**

  After this lands, the TEST-PHASE `patches:` block in `cc/kube-secrets//values/kustomize/admin-k3s/qa-de-1/a-qa-de-200/metal-operator-remote/remote/kustomization.yaml` becomes redundant and must be removed in a follow-up commit there. helm-charts only owns the cross-link target.
