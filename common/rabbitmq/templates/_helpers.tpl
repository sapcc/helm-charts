...
{{- $envAll := index . 0 }}
{{- $rabbitmq := index . 1 }}rabbit://{{ default "" $envAll.Values.global.user_suffix | print $rabbitmq.users.default.user }}:{{ $rabbitmq.users.default.password | default (tuple $envAll $rabbitmq.users.default.user | include "rabbitmq.password_for_user")  | urlquery}}@{{ include "rabbitmq.release_host" $envAll }}:{{ $rabbitmq.port | default 5672 }}{{ $rabbitmq.virtual_host | default "/" }}
{{- end}}
