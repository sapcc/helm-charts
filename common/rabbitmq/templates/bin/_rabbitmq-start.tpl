...
function bootstrap {
   ...
   rabbitmqctl add_user {{ .Values.users.default.user }} {{ .Values.users.default.password }} || true
   rabbitmqctl set_permissions {{ .Values.users.default.user }} ".*" ".*" ".*" || true

   rabbitmqctl add_user {{ .Values.users.admin.user }} {{ .Values.users.admin.password }} || true
...
{{- if .Values.metrics.enabled }}
   rabbitmqctl add_user {{ .Values.metrics.user }} {{ .Values.metrics.password }} || true
   rabbitmqctl set_permissions {{ .Values.metrics.user }} ".*" ".*" ".*" || true
   rabbitmqctl set_user_tags {{ .Values.metrics.user }} monitoring || true
{{- end }}

   rabbitmqctl change_password guest  {{ .Values.default_password }} || true
