...
def check_db_readiness(timeout=5):
    ...
    try:
        ...
        if result.returncode == 0:
            logging.info("Database server is ready to accept commands")
            return True
        else:
        ...
        logging.error(f"Error checking database readiness: {str(e)}")
        return False


def execute_sql(sql_file):
    """Execute the SQL file using mariadb client"""
    try:
        cmd = ["mariadb", f"--defaults-file={MYSQL_CLIENT_CONFIG}", "--skip-ssl", "--batch", "-e", f"source {sql_file}"]
        ...
        try:
            if check_socket_available():
                if check_db_readiness():
                    ...
                    logging.info("Socket exists but database server is not yet ready")
        except socket.error as e:
            logging.error(f"Socket connection attempt failed: {str(e)}")
    ...
    def check_file(self):
        ...
        try:
            ...
            if self.last_content_hash is None and current_content_hash is not None:
                logging.info(f"New SQL file detected: {self.file_path}")
