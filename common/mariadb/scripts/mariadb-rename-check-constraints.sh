...
# Function to check database connection with retry mechanism
check_connection() {
...
# Get check constraints for a database
get_constraints() {
    local db=$1
...
# Generate SQL for renaming constraints
generate_sql() {
    local db=$1
    local output_file=$2
...
execute_sql() {
    ...
    if [[ -s "${sql_file}" ]]; then
        ...
        mysql -h"${MYSQL_ADDRESS}" -u"${MYSQL_USER}" < "${sql_file}"
        return 0
    else
