...
| services.application.backend.ports.galera | object | `{"port":4567,"protocol":"TCP","targetPort":4567}` | Galera [replication port](https://mariadb.com/kb/en/configuring-mariadb-galera-cluster/#network-ports) |
| services.application.backend.ports.ist | object | `{"port":4568,"protocol":"TCP","targetPort":4568}` | Galera [incremental state transfer port](http://galeracluster.com/library/documentation/galera-parameters.html#ist-recv-addr) |
| services.application.backend.ports.sst | object | `{"port":4444,"protocol":"TCP","targetPort":4444}` | Galera [state snapshot transfer port](https://mariadb.com/kb/en/introduction-to-state-snapshot-transfers-ssts/) |
| services.application.backend.sessionAffinity | string | `"None"` | `None` or `ClientIP` if connections from a single client should be [routed to the same endpoint](https://kubernetes.io/docs/reference/networking/virtual-ips/#session-affinity) every time. |
...
| services.application.frontend.ports.mysql | object | `{"port":3306,"protocol":"TCP","targetPort":3306}` | MariaDB [SQL port](https://mariadb.com/kb/en/connecting-to-mariadb/#port) |
| services.application.frontend.sessionAffinity | string | `"ClientIP"` | `None` or `ClientIP` if connections from a single client should be [routed to the same endpoint](https://kubernetes.io/docs/reference/networking/virtual-ips/#session-affinity) every time. |
| services.application.frontend.sessionAffinityClientIpTimeoutSeconds | int | 10800 | [Session stickiness timeout](https://kubernetes.io/docs/reference/networking/virtual-ips/#session-affinity) for the `sessionAffinity` option |
...
| services.proxy.backend.ports.adminui | object | `{"port":6080,"protocol":"TCP","targetPort":6080}` | ProxySQL [Admin UI port](https://proxysql.com/documentation/http-web-server/) |
| services.proxy.backend.ports.proxy | object | `{"port":6032,"protocol":"TCP","targetPort":6032}` | ProxySQL [Admin SQL port](https://proxysql.com/Documentation/global-variables/admin-variables/#admin-mysql_ifaces) |
| services.proxy.backend.ports.restapi | object | `{"port":6070,"protocol":"TCP","targetPort":6070}` | ProxySQL [Rest API port](https://proxysql.com/documentation/REST-API/) |
| services.proxy.backend.sessionAffinity | string | `"None"` | `None` or `ClientIP` if connections from a single client should be [routed to the same endpoint](https://kubernetes.io/docs/reference/networking/virtual-ips/#session-affinity) every time. |
...
| services.proxy.frontend.ports.proxy | object | `{"port":3306,"protocol":"TCP","targetPort":6033}` | ProxySQL [SQL port](https://proxysql.com/Documentation/global-variables/mysql-variables/#mysql-interfaces) |
| services.proxy.frontend.sessionAffinity | string | `"None"` | `None` or `ClientIP` if connections from a single client should be [routed to the same endpoint](https://kubernetes.io/docs/reference/networking/virtual-ips/#session-affinity) every time. |
