...
#!/usr/bin/env bash
oldIFS="${IFS}"
DB_USER=${MARIADB_MONITORING_USER-monitor}
...
if ! [ -z ${COLLECT_INFO_SCHEMA_TABLES+x} ] && [ "${COLLECT_INFO_SCHEMA_TABLES}" == "enable" ]; then exporterparams[collect.info_schema.tables]+=; else exporterparams[no-collect.info_schema.tables]+=; fi
if ! [ -z ${COLLECT_INFO_SCHEMA_TABLESTATS+x} ] && [ "${COLLECT_INFO_SCHEMA_TABLESTATS}" == "enable" ]; then exporterparams[collect.info_schema.tablestats]+=; else exporterparams[no-collect.info_schema.tablestats]+=; fi
if ! [ -z ${COLLECT_PERF_SCHEMA_EVENTSSTATEMENTS+x} ] && [ "${COLLECT_PERF_SCHEMA_EVENTSSTATEMENTS}" == "enable" ]; then exporterparams[collect.perf_schema.eventsstatements]+=; else exporterparams[no-collect.perf_schema.eventsstatements]+=; fi
if ! [ -z ${COLLECT_PERF_SCHEMA_EVENTSSTATEMENTS_SUM+x} ] && [ "${COLLECT_PERF_SCHEMA_EVENTSSTATEMENTS_SUM}" == "enable" ]; then exporterparams[collect.perf_schema.eventsstatementssum]+=; else exporterparams[no-collect.perf_schema.eventsstatementssum]+=; fi
if ! [ -z ${COLLECT_PERF_SCHEMA_EVENTSWAITS+x} ] && [ "${COLLECT_PERF_SCHEMA_EVENTSWAITS}" == "enable" ]; then exporterparams[collect.perf_schema.eventswaits]+=; else exporterparams[no-collect.perf_schema.eventswaits]+=; fi
if ! [ -z ${COLLECT_PERF_SCHEMA_FILE_EVENTS+x} ] && [ "${COLLECT_PERF_SCHEMA_FILE_EVENTS}" == "enable" ]; then exporterparams[collect.perf_schema.file_events]+=; else exporterparams[no-collect.perf_schema.file_events]+=; fi
if ! [ -z ${COLLECT_PERF_SCHEMA_MEMORY_EVENTS+x} ] && [ "${COLLECT_PERF_SCHEMA_MEMORY_EVENTS}" == "enable" ]; then exporterparams[collect.perf_schema.memory_events]+=; else exporterparams[no-collect.perf_schema.memory_events]+=; fi
