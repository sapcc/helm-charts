...
function checkproxybackendlogon {
  mysql --protocol=tcp --user=${PROXYSQL_ADMIN_USER} --password=${PROXYSQL_ADMIN_PASSWORD} --host=localhost --port=${PROXYSQL_ADMIN_PORT} --prompt='Admin> ' --connect-timeout={{ $.Values.livenessProbe.timeoutSeconds.proxy }} --wait --reconnect --batch --execute="SHOW DATABASES;" | grep --silent '/opt/proxysql/data/proxysql.db'
  if [ $? -eq 0 ]; then
