...
- name: thanos-compactor.alerts
  rules:
    - alert: ThanosCompactMultipleRunning
      expr: sum by (job) (up{job=~".*thanos.*compact.*", thanos="{{ include "prometheus.name" . }}"}) > 1
      for: 5m
      labels:
    ...
    - alert: ThanosCompactHalted
      expr: thanos_compact_halted{job=~".*thanos.*compact.*", thanos="{{ include "prometheus.name" . }}"} == 1
      for: 5m
      labels:
        service: {{ default "metrics" .Values.alerts.service }}
        support_group: {{ default "observability" .Values.alerts.support_group }}
    ...
    - alert: ThanosCompactHighCompactionFailures
      expr: |
        (
          sum by (job) (rate(thanos_compact_group_compactions_failures_total{job=~".*thanos.*compact.*", thanos="{{ include "prometheus.name" . }}"}[5m]))
        /
          sum by (job) (rate(thanos_compact_group_compactions_total{job=~".*thanos.*compact.*", thanos="{{ include "prometheus.name" . }}"}[5m]))
        * 100 > 5
        )
      for: 15m
      labels:
    ...
    - alert: ThanosCompactBucketHighOperationFailures
      expr: |
        (
          sum by (job) (rate(thanos_objstore_bucket_operation_failures_total{job=~".*thanos.*compact.*", thanos="{{ include "prometheus.name" . }}"}[5m]))
        /
          sum by (job) (rate(thanos_objstore_bucket_operations_total{job=~".*thanos.*compact.*", thanos="{{ include "prometheus.name" . }}"}[5m]))
        * 100 > 5
        )
      for: 15m
      labels:
    ...
    - alert: ThanosCompactHasNotRun
      expr: (time() - max by (thanos) (max_over_time(thanos_objstore_bucket_last_successful_upload_time{job=~".*thanos.*compact.*", thanos="{{ include "prometheus.name" . }}"}[24h])))
        / 60 / 60 > 24
      labels:
        service: {{ default "metrics" .Values.alerts.service }}
        support_group: {{ default "observability" .Values.alerts.support_group }}
    ...
    - alert: ThanosCompactIsDown
      expr: up{job=~".*thanos.*compact.*", thanos="{{ include "prometheus.name" . }}"} == 0 or absent({job=~".*thanos.*compact.*", thanos="{{ include "prometheus.name" . }}"})
      for: 5m
      labels:
