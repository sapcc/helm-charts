...
- name: prometheus.alerts
  ...
  - alert: PrometheusFailedConfigReload
    ...
    expr: max_over_time(prometheus_config_last_reload_successful{prometheus="{{ .Release.Namespace }}/{{ include "prometheus.name" . }}"}[5m]) == 0
    for: 10m
    labels:
  ...
  - alert: PrometheusRuleFailures
    expr: |
      increase(prometheus_rule_evaluation_failures_total{prometheus="{{ .Release.Namespace }}/{{ include "prometheus.name" . }}"}[5m]) > 0
    for: 15m
    labels:
      service: {{ default "metrics" $root.Values.alerts.service }}
      support_group: {{ default "observability" $root.Values.alerts.support_group }}
  ...
  - alert: PrometheusMissingRuleEvaluations
    expr: |
      increase(prometheus_rule_group_iterations_missed_total{prometheus="{{ .Release.Namespace }}/{{ include "prometheus.name" . }}"}[5m]) > 0
    for: 15m
    labels:
      service: {{ default "metrics" $root.Values.alerts.service }}
      support_group: {{ default "observability" $root.Values.alerts.support_group }}
  ...
  - alert: PrometheusWALCorruption
    expr: |
      round(increase(prometheus_tsdb_wal_corruptions_total{prometheus="{{ .Release.Namespace }}/{{ include "prometheus.name" . }}"}[2h1m]) > 0)
    labels:
      service: {{ default "metrics" $root.Values.alerts.service }}
      support_group: {{ default "observability" $root.Values.alerts.support_group }}
      severity: info
  ...
  - alert: PrometheusTSDBReloadsFailing
    expr: |
      increase(prometheus_tsdb_reloads_failures_total{prometheus="{{ .Release.Namespace }}/{{ include "prometheus.name" . }}"}[3h]) > 0
    for: 4h
    labels:
      service: {{ default "metrics" $root.Values.alerts.service }}
      support_group: {{ default "observability" $root.Values.alerts.support_group }}
  ...
  - alert: PrometheusTSDBCompactionsFailing
    expr: |
      increase(prometheus_tsdb_compactions_failed_total{prometheus="{{ .Release.Namespace }}/{{ include "prometheus.name" . }}"}[3h]) > 0
    for: 4h
    labels:
      service: {{ default "metrics" $root.Values.alerts.service }}
      support_group: {{ default "observability" $root.Values.alerts.support_group }}
  ...
  - alert: PrometheusNotIngestingSamples
    expr: |
      (
        rate(prometheus_tsdb_head_samples_appended_total{prometheus="{{ .Release.Namespace }}/{{ include "prometheus.name" . }}"}[5m]) <= 0
      and
        (
          sum without(scrape_job) (prometheus_target_metadata_cache_entries{prometheus="{{ .Release.Namespace }}/{{ include "prometheus.name" . }}"}) > 0
        or
          sum without(rule_group) (prometheus_rule_group_rules{prometheus="{{ .Release.Namespace }}/{{ include "prometheus.name" . }}"}) > 0
        )
      )
    for: 10m
    labels:
  ...
  - alert: PrometheusDuplicateTimestamps
    expr: |
      rate(prometheus_target_scrapes_sample_duplicate_timestamp_total{prometheus="{{ .Release.Namespace }}/{{ include "prometheus.name" . }}"}[5m]) > 0
    for: 10m
    labels:
      service: {{ default "metrics" $root.Values.alerts.service }}
      support_group: {{ default "observability" $root.Values.alerts.support_group }}
  ...
  - alert: PrometheusOutOfOrderTimestamps
    expr: |
      rate(prometheus_target_scrapes_sample_out_of_order_total{prometheus="{{ .Release.Namespace }}/{{ include "prometheus.name" . }}"}[5m]) > 0.1
    labels:
      service: {{ default "metrics" $root.Values.alerts.service }}
      support_group: {{ default "observability" $root.Values.alerts.support_group }}
      severity: info
  ...
  - alert: PrometheusTargetSyncFailure
    expr: |
      increase(prometheus_target_sync_failed_total{prometheus="{{ .Release.Namespace }}/{{ include "prometheus.name" . }}"}[30m]) > 0
    for: 5m
    labels:
      service: {{ default "metrics" $root.Values.alerts.service }}
      support_group: {{ default "observability" $root.Values.alerts.support_group }}
  ...
  - alert: PrometheusHighQueryLoad
    expr: |
      avg_over_time(prometheus_engine_queries{prometheus="{{ .Release.Namespace }}/{{ include "prometheus.name" . }}"}[5m])
      /
      max_over_time(prometheus_engine_queries_concurrent_max{prometheus="{{ .Release.Namespace }}/{{ include "prometheus.name" . }}"}[5m])
      >
      0.8
    for: 15m
    labels:
  ...
  - alert: PrometheusNotConnectedToAlertmanagers
    ...
    # https://www.robustperception.io/alerting-on-gauges-in-prometheus-2-0 for details.
    expr: |
      max_over_time(prometheus_notifications_alertmanagers_discovered{prometheus="{{ .Release.Namespace }}/{{ include "prometheus.name" . }}"}[5m]) < 1
    for: 10m
    labels:
      service: {{ default "metrics" $root.Values.alerts.service }}
      support_group: {{ default "observability" $root.Values.alerts.support_group }}
  ...
  - alert: PrometheusErrorSendingAlerts
    expr: |
      (
        rate(prometheus_notifications_errors_total{prometheus="{{ .Release.Namespace }}/{{ include "prometheus.name" . }}"}[5m])
      /
        rate(prometheus_notifications_sent_total{prometheus="{{ .Release.Namespace }}/{{ include "prometheus.name" . }}"}[5m])
      )
      * 100
      > 1
    for: 10m
  ...
  - alert: PrometheusHighAlertRate
    expr: |
      avg by (prometheus) (rate(prometheus_notifications_sent_total{prometheus="{{ .Release.Namespace }}/{{ include "prometheus.name" . }}"}[5m]) > 50)
    for: 5m
    labels:
      service: {{ default "metrics" $root.Values.alerts.service }}
      support_group: {{ default "observability" $root.Values.alerts.support_group }}
  ...
  - alert: PrometheusNotificationQueueRunningFull
    ...
    expr: |
      (
        predict_linear(prometheus_notifications_queue_length{prometheus="{{ .Release.Namespace }}/{{ include "prometheus.name" . }}"}[5m], 60 * 30)
      >
        min_over_time(prometheus_notifications_queue_capacity{prometheus="{{ .Release.Namespace }}/{{ include "prometheus.name" . }}"}[5m])
      )
    for: 15m
    labels:
      service: {{ default "metrics" $root.Values.alerts.service }}
  ...
  - alert: PrometheusRemoteWriteDown
    expr: |
      (
        (rate(prometheus_remote_storage_samples_failed_total{prometheus="{{ .Release.Namespace }}/{{ include "prometheus.name" . }}"}[5m]))
      /
        (rate(prometheus_remote_storage_samples_total{prometheus="{{ .Release.Namespace }}/{{ include "prometheus.name" . }}"}[5m]))
      )
      * 100
      > 1
    for: 15m
  ...
  - alert: PrometheusRemoteWriteBehind
    ...
    expr: |
      (
        max_over_time(prometheus_remote_storage_highest_timestamp_in_seconds{prometheus="{{ .Release.Namespace }}/{{ include "prometheus.name" . }}"}[5m])
      - ignoring(remote_name, url) group_right
        max_over_time(prometheus_remote_storage_queue_highest_sent_timestamp_seconds{prometheus="{{ .Release.Namespace }}/{{ include "prometheus.name" . }}"}[5m])
      )
      > 120
    for: 15m
    labels:
