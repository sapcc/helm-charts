groups:
- name: thanos-compaction.alerts
  rules:
    - alert: ThanosCompactHalted
      expr: thanos_compactor_halted{app="thanos-compact", prometheus="{{ include "prometheus.name" . }}"} == 1
      for: 5m
      labels:
        context: thanos
        service: prometheus
    ...
    - alert: ThanosCompactCompactionsFailed
      expr: rate(prometheus_tsdb_compactions_failed_total{app="thanos-compact", prometheus="{{ include "prometheus.name" . }}"}[5m]) > 0
      labels:
        context: thanos
        service: prometheus
        severity: info
    ...
    - alert: ThanosCompactBucketOperationsFailed
      expr: rate(thanos_objstore_bucket_operation_failures_total{app="thanos-compact", prometheus="{{ include "prometheus.name" . }}"}[5m]) > 0
      labels:
        context: thanos
        service: prometheus
        severity: info
    ...
    - alert: ThanosCompactNotRunIn24Hours
      expr: (time() - max(thanos_objstore_bucket_last_successful_upload_time{app="thanos-compact", prometheus="{{ include "prometheus.name" . }}"}) ) /60/60 > 24
      labels:
        context: thanos
        service: prometheus
        severity: info
    ...
    - alert: ThanosComactionIsNotRunning
      expr: up{app="thanos-compact", prometheus="{{ include "prometheus.name" . }}"} == 0 or absent({app="thanos-compact", prometheus="{{ include "prometheus.name" . }}"})
      for: 5m
      labels:
        context: thanos
        service: prometheus
    ...
    - alert: ThanosComactionMultipleCompactionsAreRunning
      expr: sum(up{app="thanos-compact", prometheus="{{ include "prometheus.name" . }}"}) > 1
      for: 5m
      labels:
        context: thanos
        service: prometheus
