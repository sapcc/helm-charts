...
- name: thanos-compactor.alerts
  rules:
    - alert: ThanosCompactHalted
      expr: thanos_compact_halted{app="thanos-compactor", prometheus="{{ include "thanos.name" . }}"} == 1
      for: 5m
      labels:
        context: thanos
    ...
    - alert: ThanosCompactCompactionIsNotRunning
      expr: up{app="thanos-compactor", prometheus="{{ include "thanos.name" . }}"} == 0 or absent({app="thanos-compactor", prometheus="{{ include "thanos.name" . }}"})
      for: 5m
      labels:
    ...
    - alert: ThanosCompactMultipleCompactionsAreRunning
      expr: sum(up{app="thanos-compactor", prometheus="{{ include "thanos.name" . }}"}) > 1
      for: 5m
      labels:
        context: thanos
        service: {{ default "metrics" .Values.alerts.service }}
