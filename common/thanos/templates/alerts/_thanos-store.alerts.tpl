...
- name: thanos-store.alerts
  rules:
    - alert: ThanosStoreGrpcErrorRate
      expr: rate(grpc_server_handled_total{grpc_code=~"Unknown|ResourceExhausted|Internal|Unavailable",app="thanos-store",prometheus="{{ default (include "thanos.name" .) .Values.alerts.prometheus }}"}[5m]) > 0
      for: 5m
      labels:
        context: thanos
        service: metrics
    ...
    - alert: ThanosStoreBucketOperationsFailed
      expr: rate(thanos_objstore_bucket_operation_failures_total{app="thanos-store",prometheus="{{ default (include "thanos.name" .) .Values.alerts.prometheus }}"}[5m]) > 0
      for: 5m
      labels:
        context: thanos
        service: metrics
