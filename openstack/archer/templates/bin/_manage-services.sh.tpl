...
manage_service() {
    ...
    # Note: ports column may contain spaces like "[4000, 4005]", so we collapse spaces inside brackets before awk parsing
    SERVICE_LIST=$(${ARCHERCTL} service list -f value -c id -c name -c ip_addresses -c ports -c require_approval -c visibility -c proxy_protocol -c protocol -c provider 2>/dev/null | sed 's/, /,/g' || true)
    EXISTING=$(echo "$SERVICE_LIST" | awk -v name="$name" '$2 == name { print; exit }')
