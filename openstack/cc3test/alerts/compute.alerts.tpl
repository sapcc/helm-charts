...
- name: cc3test-vcenter.alerts
  ...
  - alert: OpenstackComputeKVMCanaryVolumeAttachFailed
    expr: |
    ...
    labels:
      ...
      service: "{{`{{ $labels.service }}`}}"
      context: "{{`{{ $labels.service }}`}}"
      meta: "Openstack Compute KVM Canary: {{`{{ $labels.name }}`}} is down, see report for more details"
      dashboard: cc3test-bb-vc-canary-status?var-service={{`{{ $labels.service }}`}}
      persesDashboard: "https://perses.{{ .Values.global.region }}.{{ .Values.global.tld }}/projects/observability/dashboards/cc3test-bb-vc-canary-status?var-service={{`{{ $labels.service }}`}}"
      playbook: "docs/support/playbook/nova/alerts/cc3test-alert-kvm-attach-volume/"
      report: "cc3test/admin/object-storage/swift/containers/cc3test/objects/{{`{{ $labels.base64path }}`}}"
    annotations:
      description: "Openstack Compute KVM Canary: {{`{{ $labels.name }}`}} is down, see report for more details"
      summary: "Openstack Compute KVM Canary: {{`{{ $labels.name }}`}} is down, see report for more details"


  - alert: OpenstackComputeKVMCanaryVolumeAttachFlapping
    ...
    labels:
      ...
      service: "{{`{{ $labels.service }}`}}"
      context: "{{`{{ $labels.service }}`}}"
      meta: "Openstack Compute KVM Canary is flapping for 2 hours, see last three reports for more details"
      dashboard: cc3test-bb-vc-canary-status?var-service={{`{{ $labels.service }}`}}
      persesDashboard: "https://perses.{{ .Values.global.region }}.{{ .Values.global.tld }}/projects/observability/dashboards/cc3test-bb-vc-canary-status?var-service={{`{{ $labels.service }}`}}"
      playbook: "docs/support/playbook/nova/alerts/cc3test-alert-kvm-attach-volume/"
      report: "cc3test/admin/object-storage/swift/containers/cc3test/objects/{{`{{ $labels.base64path }}`}}"
    annotations:
      description: "Openstack Compute KVM Canary is flapping for 2 hours, see last three reports for more details"
      summary: "Openstack Compute KVM Canary is flapping for 2 hours, see last three reports for more details"
