{
    ...
    "os_load-balancer_api:member:put_cross_pool_members": "{{- if .Values.early_lbaas_cpbmu_access_project_ids }}({{ range $i, $projectID := .Values.early_lbaas_cpbmu_access_project_ids }}{{ if $i }} or {{ end }}project_id:{{ $projectID }}{{ end }}) and role:lbaas_cpbmu and rule:owner{{- else }}!{{- end }}"
}
