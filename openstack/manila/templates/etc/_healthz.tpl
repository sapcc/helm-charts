...
/etc/manila/rabbitmqadmin -H {{ .Release.Name }}-rabbitmq -u {{ .Values.rabbitmq.users.admin.user | include "resolve_secret" }} -p {{ .Values.rabbitmq.users.admin.password | include "resolve_secret" }} list bindings | grep $(hostname)
export BINDINGS_EXIT_CODE=$?
# rabbitmq has no uncollected messages
/etc/manila/rabbitmqadmin -H {{ .Release.Name }}-rabbitmq -u {{ .Values.rabbitmq.users.admin.user | include "resolve_secret" }} -p {{ .Values.rabbitmq.users.admin.password | include "resolve_secret" }} list queues -f tsv| grep $(hostname)|cut -f 2|grep -w 0
