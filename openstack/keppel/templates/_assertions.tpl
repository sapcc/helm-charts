...
{{- if ne (kindOf $p.members) "slice" -}}
  {{- fail (printf "expected list for \"members\" attribute of peer group, but got %#v") $p.members -}}
{{- end -}}
...
{{- if ne (kindOf $p.exclude_from_anycast) "slice" -}}
  {{- fail (printf "expected list for \"exclude_from_anycast\" attribute of peer group, but got %#v") $p.exclude_from_anycast -}}
{{- end -}}
{{- if ne (kindOf $p.exclude_from_pull_delegation) "slice" -}}
  {{- fail (printf "expected list for \"exclude_from_anycast\" attribute of peer group, but got %#v") $p.exclude_from_pull_delegation -}}
{{- end -}}
