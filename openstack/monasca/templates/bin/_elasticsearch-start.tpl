...
function process_config {
  cp -f /monasca-etc/elasticsearch-elasticsearch.yaml /elasticsearch/config/elasticsearch.yaml
  cp -f /monasca-etc/elasticsearch-log4j2.properties /elasticsearch/config/log4j2.properties
  cp -f /monasca-etc/elasticsearch-jvm.options /elasticsearch/config/jvm.options
}
...
function start_application {
  ...
  (sleep 180; curl -u {{.Values.monasca_elasticsearch_admin_user}}:{{.Values.monasca_elasticsearch_admin_password}} -XPUT localhost:{{.Values.monasca_elasticsearch_port_internal}}/_cluster/settings -d '{"transient": { "cluster.routing.allocation.enable": "all" } }') &
