...
sed -i "s|TEMPEST_ADMIN_PASSWORD_PLACEHOLDER|${OS_PASSWORD}|g" /tmp/tempest_accounts.yaml

cp /{{ .Chart.Name }}-etc/tempest_deployment_config.json /tmp/tempest_deployment_config.json
sed -i "s|TEMPEST_ADMIN_PASSWORD_PLACEHOLDER|${OS_PASSWORD}|g" /tmp/tempest_deployment_config.json

cp /{{ .Chart.Name }}-etc/tempest_extra_options /tmp/tempest_extra_options
sed -i "s|TEMPEST_ADMIN_PASSWORD_PLACEHOLDER|${OS_PASSWORD}|g" /tmp/tempest_extra_options
...
function start_tempest_tests {
  ...
  openstack object create reports/{{ index (split "-" .Chart.Name)._0 }} /tmp/tempest-log.tar.gz --name $(echo $OS_REGION_NAME)-epoxy-$(echo $MYTIMESTAMP)-log.tar.gz
  openstack object create reports/{{ index (split "-" .Chart.Name)._0 }} /tmp/tempest-log.tar.gz --name $(echo $OS_REGION_NAME)-epoxy-log-latest.tar.gz
  openstack object create reports/{{ index (split "-" .Chart.Name)._0 }} /tmp/report.html --name $(echo $OS_REGION_NAME)-epoxy-$(echo $MYTIMESTAMP).html
  openstack object create reports/{{ index (split "-" .Chart.Name)._0 }} /tmp/report.html --name $(echo $OS_REGION_NAME)-epoxy-latest.html
  openstack object create reports/{{ index (split "-" .Chart.Name)._0 }} /tmp/report.json --name $(echo $OS_REGION_NAME)-epoxy-$(echo $MYTIMESTAMP).json
