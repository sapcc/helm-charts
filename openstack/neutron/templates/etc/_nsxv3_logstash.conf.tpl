...
filter {
  if [type] == "esxi" and "FIREWALL_PKTLOG" in [message] {
    ...
    if "_grokparsefailure" not in [tags] {
      ruby {
        init => 'require "sequel"; $rc = Sequel.connect("jdbc:mysql://{{include "neutron.db_service" .}}/{{.Values.db_name}}?user=%s&password=%s" % [ENV["NEUTRON_DB_USER"], ENV["NEUTRON_DB_PASSWORD"]])'
        code => '
