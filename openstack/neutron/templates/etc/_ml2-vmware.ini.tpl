...
[securitygroup]
firewall_driver = {{required "A valid ml2-vmware config required!" $hypervisor.firewall}}

[ml2_vmware]
# Hostname or ip address of vmware vcenter server
vsphere_hostname = {{required "A valid ml2-vmware config required!" $hypervisor.host}}
cluster_name = {{required "A valid ml2-vmware config required!" $hypervisor.cluster_name}}

# Login username and password of vcenter server
vsphere_login = {{required "A valid ml2-vmware config required!" $hypervisor.username | replace "$" "$$"}}
vsphere_password = {{required "A valid ml2-vmware config required!" $hypervisor.password | replace "$" "$$"}}
