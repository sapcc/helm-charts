...
$CURL_CMD -s --netrc-file "${NETRC_FILE}" "${CLUSTER_HOST}/"
if [ $? -ne 0 ]; then
...
if [ "${DATA_STREAM_ENABLED}" = true ]; then
   for e in ${DATA_STREAMS}; do
     ...
     if  grep -q "$e" "${TMPPATH}/${e}-${DS_TEMPLATE}" ; then
         $CURL_CMD --netrc-file "${NETRC_FILE}" -H 'Content-Type: application/json' -XPUT "${CLUSTER_HOST}/_index_template/${e}-datastream" -d @"${TMPPATH}/${e}-${DS_TEMPLATE}"
   ...
   #
   # After a new ds ism template is installed, it is only active after a rollover of the datastream, This means after the default 24h period or a manual rollover, which can also be triggered by a job.
   ...
   for e in ${DATA_STREAMS}; do
     ...
     if [ "${POLICY_RETURN_CODE}" -eq 404 ]; then
        ...
        $CURL_CMD --netrc-file "${NETRC_FILE}" -XPUT "${CLUSTER_HOST}/_plugins/_ism/policies/ds-${e}-ism" -H 'Content-Type: application/json' -d @"${TMPPATH}/${DS_ISM_TEMPLATE}"
        ...
        export EXISTING_DS=`$CURL_CMD -s --netrc-file "${NETRC_FILE}" -XGET "${CLUSTER_HOST}/_data_stream"|jq .data_streams[].name|tr  -d \"`
        echo -e "\nExisting datastreams: $EXISTING_DS\n"
        echo "$EXISTING_DS" | grep -q "^$e$"
        if [ $? -eq 1 ]; then
           echo -e "\nCreating datastream $e\n"
           $CURL_CMD --netrc-file "${NETRC_FILE}" -XPUT "${CLUSTER_HOST}/_data_stream/${e}"
           # Assign ISM policy to newly created datastream
           $CURL_CMD --header 'content-type: application/JSON' --silent --netrc-file "${NETRC_FILE}" -XPOST "${CLUSTER_HOST}/_plugins/_ism/add/.ds-${e}-000001" -d "{ \"policy_id\": \"ds-${e}-ism\" }"
        else
       ...
       export CLUSTER_RETENTION_RESPONSE=$($CURL_CMD -s --netrc-file "${NETRC_FILE}" -XGET "${CLUSTER_HOST}/_plugins/_ism/policies/ds-${e}-ism")
       ...
       if [ "${FILE_RETENTION_SCHEMA_VERSION}" -gt "${CLUSTER_RETENTION_SCHEMA_VERSION}" ]; then
         ...
         $CURL_CMD --netrc-file "${NETRC_FILE}" -XPUT "${CLUSTER_HOST}/_plugins/_ism/policies/ds-${e}-ism?if_seq_no=${CLUSTER_RETENTION_SEQ_NUMBER}&if_primary_term=${CLUSTER_RETENTION_RUN_PRIM_TERM}" -H 'Content-Type: application/json' -d @"${TMPPATH}/${DS_ISM_TEMPLATE}"
         cat "${TMPPATH}/${DS_ISM_TEMPLATE}"
         export NEW_CLUSTER_RETENTION_SCHEMA_VERSION=$($CURL_CMD -s --netrc-file "${NETRC_FILE}" -XGET "${CLUSTER_HOST}/_plugins/_ism/policies/ds-${e}-ism"|jq .policy.schema_version?)
