...
<filter	kubernetes.var.log.containers.kube-system-nginx-ingress-controller**>
  ...
  <parse>
    @type grok
    grok_pattern %{GREEDYDATA}]%{SPACE}"%{WORD:method}%{SPACE}%{IMAGE_METHOD:path}%{NOTSPACE}%{SPACE}%{NOTSPACE}"%{SPACE}%{NUMBER:response}
    grok_pattern %{GREEDYDATA}]%{SPACE}"%{WORD:method}%{SPACE}%{IMAGE_METHOD:path}%{GREEDYDATA}\/1.1"%{SPACE}%{NUMBER:response}
