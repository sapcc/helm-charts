...
function start_application {
  ...
  cat <(crontab -l) <(echo "0 1,13 * * * export LC_ALL=C.UTF-8; export LANG=C.UTF-8; curl -s -u {{.Values.global.elk_elasticsearch_admin_user}}:{{.Values.global.elk_elasticsearch_admin_password}} http://localhost:9200/_aliases?pretty=true|grep kuber|grep '20[3-9]'|awk '{ print $1}'|tr -d '"'|xargs -I {} curl -u {{.Values.global.elk_elasticsearch_admin_user}}:{{.Values.global.elk_elasticsearch_admin_password}} -X DELETE "http://{{.Values.endpoint_host_internal}}:{{.Values.http_port}}/{}?pretty"
  cat <(crontab -l) <(echo "0 1,13 * * * export LC_ALL=C.UTF-8; export LANG=C.UTF-8; curl -s -u {{.Values.global.elk_elasticsearch_admin_user}}:{{.Values.global.elk_elasticsearch_admin_password}} http://localhost:9200/_aliases?pretty=true|grep kuber|grep '21[0-9]'|awk '{ print $1}'|tr -d '"'|xargs -I {} curl -u {{.Values.global.elk_elasticsearch_admin_user}}:{{.Values.global.elk_elasticsearch_admin_password}} -X DELETE "http://{{.Values.endpoint_host_internal}}:{{.Values.http_port}}/{}?pretty"
  cat <(crontab -l) <(echo "0 1,13 * * * export LC_ALL=C.UTF-8; export LANG=C.UTF-8; curl -s -u {{.Values.global.elk_elasticsearch_admin_user}}:{{.Values.global.elk_elasticsearch_admin_password}} http://localhost:9200/_aliases?pretty=true|grep kuber|grep '202[4-9]'|awk '{ print $1}'|tr -d '"'|xargs -I {} curl -u {{.Values.global.elk_elasticsearch_admin_user}}:{{.Values.global.elk_elasticsearch_admin_password}} -X DELETE "http://{{.Values.endpoint_host_internal}}:{{.Values.http_port}}/{}?pretty"
