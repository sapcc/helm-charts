...
- name: servicemesh.alerts
  ...
  - alert: LinkerdProxySidecarCertificateExpiringSoon
    ...
    labels:
      ...
      context: servicemesh
      meta: "Linkerd proxy sidecar certificate in pod {{`{{ $labels.namespace }}`}}/{{`{{ $labels.pod_name }}`}} is expiring in less than 2h."
      #playbook: 'docs/support/playbook/kubernetes/linkerd'
      no_alert_on_absence: "true"
    annotations:
      summary: Linkerd proxy sidecar certificate expiring
      description: "Linkerd proxy sidecar certificate in pod {{`{{ $labels.namespace }}`}}/{{`{{ $labels.pod_name }}`}} is expiring in less than 2h. Immediate action is required or network connectivity might be lost."
  - alert: LinkerdProxySidecarCertificateExpiring
    ...
    labels:
      ...
      severity: warning
      context: servicemesh
      meta: "Linkerd proxy sidecar certificate in pod {{`{{ $labels.namespace }}`}}/{{`{{ $labels.pod_name }}`}} is expiring in less than 24h."
      #playbook: 'docs/support/playbook/kubernetes/linkerd'
      no_alert_on_absence: "true"
    annotations:
      summary: Linkerd proxy sidecar certificate expiring
      description: "Linkerd proxy sidecar certificate in pod {{`{{ $labels.namespace }}`}}/{{`{{ $labels.pod_name }}`}} is expiring in less than 24h. Immediate action is required or network connectivity might be lost."
