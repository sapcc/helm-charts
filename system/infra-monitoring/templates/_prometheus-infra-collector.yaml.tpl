...
- job_name: 'redfish/fw'
  ...
  http_sd_configs:
    - url: {{ .Values.http_sd_configs.netbox_url }}/devices/?custom_labels=job=redfish/fw&target=mgmt_only&status=active&role=server&tenant=converged-cloud&tagn=no-redfish&region={{ .Values.global.region }}
...
- job_name: 'win-exporter-ad'
  ...
  scrape_timeout: {{$values.scrapeTimeout}}
  http_sd_configs:
    - url: {{ .Values.http_sd_configs.netbox_url }}/devices/?custom_labels=job=redfish/fw&target=primary_ip&status=active&role=server&tenant=converged-cloud&platform=windows-server&tag=active-directory-domain-controller&region={{ .Values.global.region }}
  metrics_path: /metrics
  relabel_configs:
    - source_labels: [job]
