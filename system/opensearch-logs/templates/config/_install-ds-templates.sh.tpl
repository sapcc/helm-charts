...
####
echo "machine ${CLUSTER_HOST} login ${ADMIN_USER} password ${ADMIN_PASSWORD}" > "${NETRC_FILE}"
...
if [ $? -ne 0 ]; then
  ...
  echo "machine ${CLUSTER_HOST} login ${ADMIN2_USER} password ${ADMIN2_PASSWORD}" > "${NETRC_FILE}"
  curl -s --netrc-file "${NETRC_FILE}" "${CLUSTER_HOST}/"
  if [ $? -ne 0 ]; then
